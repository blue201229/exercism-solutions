...
impl BowlingGame {
    ...
    pub fn roll(&mut self, pins: u16) -> Result<(), Error> {
        // Check if game is already complete (10 frames finished)
    ...
    fn get_first_roll_of_frame(&self, frame: usize) -> u16 {
        let mut current_frame = 0;
        let mut roll_index = 0;

        while roll_index < self.rolls.len() && current_frame < frame {
            if current_frame == 9 {
                break;
            }
            ...
            if self.rolls[roll_index] == 10 {
                ...
                // Not a strike, so 2 rolls per frame
                current_frame += 1;
                roll_index += 2;
            }
        }
        println!("roll_idx {frame} - {roll_index}");
        self.rolls[roll_index]
    }

    fn get_frame_number(&self) -> usize {
        self.get_frame_info().0
    }
